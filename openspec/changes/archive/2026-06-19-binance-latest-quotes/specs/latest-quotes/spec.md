...
The gateway SHALL fetch uncached Binance Spot prices through one public
`GET /api/v3/ticker/price` request containing all required provider symbols.

#### Scenario: Both required quotes need refresh

- **WHEN** enabled `BTC/USD` and `ETH/USD` have no reusable cache entries
- **THEN** the adapter requests `BTCUSD` and `ETHUSD` in one Binance batch call
...
The gateway SHALL convert Binance timeout, transport, non-success status, and invalid-response
